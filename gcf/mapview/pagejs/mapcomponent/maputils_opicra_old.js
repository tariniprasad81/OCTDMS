const maputils = (function ($, olMap) {
  ...
  function tankononmap(
    ...
    var vectorSourcepoint = new ol.source.Vector();
    var vectorSourcepoint2 = new ol.source.Vector();
    ...
    if (latprovided != 0 || lngprovided != 0) {
      vectorSourcepoint2.addFeatures(featurespoint2);
      new ol.layer.Vector({
        map: olMap,
        source: vectorSourcepoint2,
        style: [styleMarker2],

        updateWhileAnimating: true,
        updateWhileInteracting: true,
      });
      olMap.getView().fit(vectorSourcepoint2.getExtent(), {
        size: olMap.getSize(),
        maxZoom: 7,
      });
    }
      ...
      success: function (response) {
        ...
        if (response.status == 1) {
          maputils.tankononmap(
            dataObj.latitudeProvided,
            dataObj.longitudeProvided,
